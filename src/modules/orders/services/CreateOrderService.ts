...
class CreateOrderService {
  ...
  public async execute({ customer_id, products }: IRequest): Promise<Order> {
    ...
    if (!existentProducts.length) {
      throw new AppError('Could not find any products with those ids');
    }
    ...
    const orderedProductsQuanity = products.map(product => ({
      id: product.id,
      quantity:
        existentProducts.filter(p => p.id === product.id)[0].quantity -
        product.quantity,
    ...
    await this.productsRepository.updateQuantity(orderedProductsQuanity);
